...
#: Default chunk size in characters. Tunable by Optuna.
RECURSIVE_CHUNK_SIZE: int = 500

#: Default chunk overlap in characters. Tunable by Optuna.
...
def recursive_split_text(
    ...
    Args:
        ...
        chunk_overlap:
            ...
            next piece. Must be in ``[0, chunk_size)``.
        separators:
    ...
    Raises:
        ValueError:
            If ``chunk_size < 1`` or ``chunk_overlap`` is outside
            ``[0, chunk_size)``.
    """
    if chunk_size < 1:
...
class RecursiveChunker:
    ...
    def chunk_profile(
        ...
        for i, entry in enumerate(experience.get("entries") or []):
            sections.append((f"experience_{i}", _entry_to_text(entry)))

        education = profile.get("education") or {}
        for i, entry in enumerate(education.get("entries") or []):
