...
| **Retrieval Mode** | **Requirement-Level Group Retrieval (default `K=3 per REQ`, cap `15`)** | **Active (2026-07-25, DEC-038)** | **Concatenates sub-query strings into 1 requirement query vector, running 1 single vector search pass per requirement with local FastEmbedder ONNX model (`BAAI/bge-base-en-v1.5`). Achieved 65.00% Context Relevance precision and 92.26% Answer Relevance (exceeding production target of >= 90%). Implementation in `recruiter/src/rag/per_req_retrieval.py`.** |
...
| **Per-Candidate Evidence Retrieval** | **Requirement-Level Group Retrieval over DocumentAware chunks** | **Active (2026-07-25, DEC-038)** | **Each REQ retrieves its top-K chunks from the candidate's DocumentAware index using 1 combined requirement vector; scoring engine consumes retrieved chunks for the rubric-bound LLM judge.** |
