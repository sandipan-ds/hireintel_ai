...
## 2026-07-18 — BGE Query Prefixing & Dynamic Comparative Chat & Cloud Run Tuning (DEC-037)

### What changed

- **BGE Query Prefixing:** Added automatic query instruction prefixing (`Represent this sentence for searching relevant passages: `) inside the local FastEmbed engine for queries flagged as `RETRIEVAL_QUERY`.
- **Dynamic Comparative RAG Chat:** Refactored the dashboard chat API to dynamically parse query strings for candidate ranks, numbers, or name mentions, and fetch relevant chunks *only* for the candidates in the comparison.
- **BYOK Config Field Sync:** Synced the `Judge Model` configuration input on dashboard and candidate views using `localStorage` event hooks.
- **Removed Recommendations Sidebar:** Cleaned up the sidebars by removing model recommendation containers.
- **Always-Allocated CPU Deployed:** Configured Cloud Run with `--no-cpu-throttling` to ensure background threads and FastEmbed ONNX inference runs at maximum vCPU speeds.

### Why

- The local `BAAI/bge-base-en-v1.5` model requires specific instruction prefixes for query search. Without it, RAG context relevance and faithfulness metrics dropped to 0%.
- Restricting chat queries to a single candidate made comparative analyses impossible. Exposing a lightweight candidate lookup registry and fetching chunks for compared candidates dynamically keeps contexts clean while enabling cross-candidate queries.
- CPU throttling in serverless containers causes long latency delays on CPU-based FastEmbed ONNX runs during RAG and background evaluations.

### Affected files

- **Modified:** `recruiter/src/rag/local_embedder.py` — added search instructions formatting.
- **Modified:** `recruiter/src/api/dashboard.py` — added target entity parsing, cross-candidate retrieval, and registry index block.
- **Modified:** `recruiter/src/templates/candidate.html` — synced Judge Model UI inputs, deleted recommendations box.
- **Modified:** `recruiter/src/templates/dashboard.html` — synced Judge Model UI inputs, deleted recommendations box.
- **Modified:** `recruiter/src/templates/recruiter.html` — deleted recommendations box.

---

## 2026-07-17 — FastEmbed Everywhere Local ONNX Embedding Engine (DEC-036)

### What changed

- **Local Embedding Engine:** Replaced the external Google Gemini API embedding REST wrapper with the local `fastembed` ONNX Runtime engine.
- **Pre-downloaded Weights:** Configured `FASTEMBED_CACHE_PATH` inside the Docker container to cache model files at build time, eliminating runtime download latency.
- **Offline Safety Flags:** Set `HF_HUB_OFFLINE=1` and `TRANSFORMERS_OFFLINE=1` at serve-time inside the container to prevent any Hugging Face server queries.
- **Thread-safe Logging:** Added a thread-safe `threading.Lock()` to `save_progress` in `score_batch_composed.py` to prevent Windows permission conflicts under concurrent execution.

### Why

- The Gemini API free-tier suffered from strict rate limits (15 RPM and low concurrency) that caused persistent 429 errors during high-throughput evaluation runs.
- `fastembed` performs CPU inference locally without PyTorch or sentence-transformers dependencies, reducing memory usage, avoiding cold start delays on Cloud Run, and eliminating all HTTP dependency/quota constraints.

### Affected files

- **Modified:** `recruiter/src/rag/gemini_embedder.py` — rewrote class to wrap FastEmbed BGE model.
- **Modified:** `Dockerfile` — added model pre-download caching and offline flags.
- **Modified:** `requirements.prod.txt` & `requirements.txt` — added `fastembed`.
- **Modified:** `recruiter/score_batch_composed.py` & root copy — added concurrency Lock to `save_progress`.
