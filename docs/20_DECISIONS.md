...
| DEC-035 | 2026-07-13 | **Revert active chunker to DocumentAwareChunker; upgrade embedding model to BAAI/bge-base-en-v1.5; switch retrieval from threshold to top-K.** Root cause analysis (BUG-RC-001) revealed that RecursiveChunker (DEC-019) disperses evidence uniformly across overlapping text blobs — binary sub-queries (Agile, Documentation, Cross-functional, UAT) were returning 0 for 56–89% of candidates despite the evidence being present, because the 1000-char overlapping chunks gave the LLM no focused evidence. DocumentAwareChunker reads the already-normalized structured profile JSON (one chunk per job entry, section_type tags) so the LLM receives dense, targeted evidence. The `all-MiniLM-L6-v2` embedding model (384-dim, STS-trained) was replaced with `BAAI/bge-base-en-v1.5` (768-dim, retrieval-trained on MS-MARCO/BEIR) for better sub-query → resume-chunk alignment. Cosine threshold retrieval replaced with top-K (default K=10 per REQ) to guarantee evidence is always retrieved. HPO scripts (Optuna, grid sweep, judge eval) and MLflow tracking deleted as they were tuned to threshold parameters that are now irrelevant. Stale data (embeddings, scores, optuna studies, mlflow artifacts, hireintel.db) purged and rebuilt. Parallel scoring (n_workers=10, `ThreadPoolExecutor` per REQ) retained. | Accepted (supersedes DEC-019, DEC-017's threshold-based retrieval mode) | `14_MODEL_REGISTRY.md`, `09_CHUNKING_AND_METADATA_SPEC.md`, `22_ARCHITECTURE_CHANGELOG.md`, `24_TROUBLESHOOTING.md` |
...
## DEC-036: FastEmbed Everywhere (Local ONNX)
...
During high-concurrency resume evaluation runs, the embedding pipeline was hitting severe `429 Too Many Requests` rate limits when calling the Google Gemini embedding REST API. While key rotation was implemented, the free tier limits (15 RPM and low concurrency) were exhausted during subquery caching and index building. A fully local option using PyTorch and SentenceTransformers had been previously deprecated due to a large container footprint (~4 GB) and long cold start times (~60-90 seconds) on Cloud Run.

### Decision

Migrate the entire embedding pipeline (both local dev/scoring runs and cloud serving/indexing paths) to **FastEmbed** running locally via ONNX Runtime:
1. **Drop-in replacement:** Modify `GeminiEmbedder` to wrap `fastembed.TextEmbedding` using the `BAAI/bge-base-en-v1.5` model.
2. **Automatic Model Mapping:** If a model name like `gemini-embedding-001` or `text-embedding-004` is configured, transparently map it to `BAAI/bge-base-en-v1.5`.
3. **Build-Phase Baking:** Set `FASTEMBED_CACHE_PATH=/app/.cache/fastembed` and pre-download the ONNX model files during the Docker build phase, ensuring the image is self-contained.
4. **Offline Mode:** Enable `HF_HUB_OFFLINE=1` and `TRANSFORMERS_OFFLINE=1` at runtime in the container to guarantee zero external requests for embeddings.
5. **Thread-Safe lock:** Implement a lock around progress file saving in `score_batch_composed.py` to prevent Windows permission issues during concurrent runs.

### Alternatives Considered

- **Keep Gemini API with strict proactive sleep intervals:** Rejected. Forcing a 4-second delay per embedding request would make index building and subquery caching extremely slow, and still run the risk of rate-limiting if keys are shared.
- **Switch to a smaller cloud model:** Rejected. Does not solve the rate-limit or API key management issues.
- **SentenceTransformers + PyTorch:** Rejected. Large package download sizes and cold-start latency issues.

### Tradeoffs

- **Slight Container size increase:** The container image increases by ~210 MB due to the baked BGE ONNX model, bringing the final size to ~1 GB. This is still highly performant for serverless cold-starts compared to the legacy ~4 GB PyTorch container.

### Related Documents

`14_MODEL_REGISTRY.md`, `22_ARCHITECTURE_CHANGELOG.md`, `Dockerfile`, `requirements.prod.txt`, `requirements.txt`.

---

## DEC-037: BGE Query Prefixing & Dynamic Comparative Chat & Cloud Run Tuning
...
After migrating from cloud-based Gemini embeddings to the local `BAAI/bge-base-en-v1.5` model (DEC-035/DEC-036), evaluation runs showed that the RAG Context Relevance and Groundedness metrics dropped to 0.0%. BGE-base-en-v1.5 requires retrieval queries to be prepended with a specific search instruction prefix (`Represent this sentence for searching relevant passages: `) to locate matching text passages correctly. Without this prefix, the query embeddings were misaligned, leading to irrelevant chunk retrieval. Furthermore, the candidate chat dashboard was limited to a single candidate's chunks, making it impossible to perform comparative queries across different candidates or ranks without stuffing the entire pool context. Lastly, the serverless Cloud Run instance suffered latency drops due to CPU throttling during background scoring runs.

### Decision

1. **BGE Query Prefixing:** Modify `FastEmbedder.encode` in `local_embedder.py` to automatically prepend retrieval sub-queries with `"Represent this sentence for searching relevant passages: "` if the task type is `RETRIEVAL_QUERY`.
2. **Always-Allocated CPU:** Deployed the Cloud Run container with the `--no-cpu-throttling` flag to prevent CPU power-downs on background processing loops.
3. **Dynamic Target Candidate Resolution:** Enhance the chat endpoint (`dashboard.py::chat_with_candidate`) to parse query strings for target ranks (e.g. `Rank 1`), candidate numbers (e.g. `CAND_0003`), or candidate names.
4. **Dynamic RAG Context Builder:** Update the chat endpoint to dynamically query, retrieve, and tag chunks/evidence *only* for the candidates involved in the comparison, combined with a lightweight `CANDIDATE RANKING REGISTRY` lookup index passed in the system prompt.
5. **BYOK UI Sync & Cleanup:** Restored the `Judge Model` configuration input on candidate and dashboard templates, synced its value via `localStorage`, and removed the `Recommendations · multimodal only` container block.

### Alternatives Considered

- **Passing top 10/20 candidate summaries to the prompt:** Rejected. Stuffing candidate summaries into the context window for every query introduces unnecessary noise, degrades focus, and increases token cost.
- **Passing all role resume documents in a single large context:** Rejected. Exceeds prompt limits and degrades instruction compliance.

### Tradeoffs

- **Slightly more complex query pre-processing:** Programmatic parsing of rank and name entities in user queries adds a regex parsing step in Python, but this is negligible (~1ms) compared to prompt latency and saves significant context tokens.

### Related Documents

`local_embedder.py`, `dashboard.py`, `score_batch_composed.py`, `candidate.html`, `dashboard.html`, `recruiter.html`, `walkthrough_7.md`.
