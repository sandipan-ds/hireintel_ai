...
### Added — 2026-07-18: BGE Query Prefixing & Dynamic Comparative Chat & Cloud Run Tuning (DEC-037)

- **BGE Query Prefixing:** Added automatic query instruction prefix formatting for local FastEmbed retrieve queries, fixing the RAG context relevance and faithfulness correctness score drop.
- **Dynamic Comparative RAG Chat:** Chat dashboard dynamically resolves target candidates by rank/number/name, queries their matching evidence chunks, and compares them using a clean prompt registry map.
- **Always-Allocated CPU Tuning:** Configured `--no-cpu-throttling` on GCP Cloud Run to prevent container CPU power-downs and optimize local ONNX-based FastEmbed performance.
- **BYOK Config UI & Sidebar Cleanup:** Synced the `Judge Model` configuration across pages using `localStorage` hooks and removed model recommendations boxes.
