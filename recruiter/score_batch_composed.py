...
def run_rag_evaluation(role: str, judge_model: str, output_dir: Path) -> None:
    """Run sample-based RAG correctness evaluation on the scored candidates.

    Evaluates three standard RAG metrics using a Judge LLM:
      1. Context Relevance (Precision) — does the retrieved chunk help
         answer the sub-query?
      2. Faithfulness (Groundedness) — is the scoring explanation
         grounded in the source evidence?
      3. Answer Relevance — does the explanation actually answer the
         sub-query?

    Key design decisions (fixes for the 0.0% bug):
      - SKIP blocked requirements (``rubric_trace`` is empty when
        retrieval returned zero evidence upstream). Instead, scan ALL
        requirements for each candidate and pick the first N that have
        actual ``rubric_trace.sub_scores`` data.
      - Use the ``justification`` field from ``rubric_trace`` (auto-
        generated by ``CachedScoringTrace.justification`` property)
        which synthesises evidence, cited text, and anchor descriptions
        into a proper explanation paragraph.
      - Match each sub-query's ``closest_evidence`` to that specific
        sub-query for Context Relevance, rather than reusing one global
        chunk for all sub-queries.
    """
    import os
    import random
    import re
    ...
    if not api_key:
        logger.warning("RECRUITER_API_KEY not set. Skipping RAG correctness evaluation.")
        return
    ...
    #
    # Instead of blindly taking reqs[:2] (which are often ALL blocked
    # when retrieval found zero evidence), scan the full requirement
    # list and collect the first N that have non-empty rubric_trace
    # with usable sub_scores data.
    MAX_REQS_PER_CANDIDATE = 3

    def _collect_evaluable_reqs(
        reqs: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """Return up to MAX_REQS_PER_CANDIDATE requirements that have
        rubric_trace with at least one sub-score containing evidence."""
        ...
        for r in reqs:
            ...
            if has_evidence:
                evaluable.append(r)
            if len(evaluable) >= MAX_REQS_PER_CANDIDATE:
                break
        return evaluable

    # ----- main evaluation loop -----
    total_reqs_evaluated = 0
    total_reqs_skipped_blocked = 0
    total_reqs_skipped_no_trace = 0
    ...
    for cand in sampled_candidates:
        ...
        # Count diagnostics for this candidate
        n_blocked = sum(1 for r in reqs if r.get("blocked"))
        n_no_trace = sum(
            1 for r in reqs
            if not r.get("blocked") and not (r.get("rubric_trace") or {}).get("sub_scores")
        )
        total_reqs_skipped_blocked += n_blocked
        total_reqs_skipped_no_trace += n_no_trace

        evaluable = _collect_evaluable_reqs(reqs)
        if not evaluable:
            logger.info(
                "⚖️ Candidate %s: %d/%d reqs blocked (zero evidence), "
                "%d have no rubric trace — nothing to evaluate.",
                candidate_id, n_blocked, len(reqs), n_no_trace,
            )
            continue

        for req in evaluable:
    ...
    eval_results: Dict[str, Any] = {
        ...
        "reqs_skipped_blocked": total_reqs_skipped_blocked,
        "reqs_skipped_no_trace": total_reqs_skipped_no_trace,
        "judge_calls_made": {
            "context_relevance": len(context_relevance_scores),
            "faithfulness": len(faithfulness_scores),
            "answer_relevance": len(answer_relevance_scores),
        },
    }
    ...
    if rate_limit_encountered:
        ...
        logger.warning(
            "1. In the Recruiter Wizard UI, lower Candidate Concurrency "
            "and Requirement Concurrency."
        )
        logger.warning(
            "2. Or supply a paid/higher-tier API key with higher RPM limits."
        )
        logger.warning("=" * 70)
    ...
    try:
        ...
        if no_evaluable_data:
            ...
            logger.info(
                "Evaluated %d requirements across %d candidates "
                "(%d blocked, %d no-trace skipped).",
                total_reqs_evaluated, sample_size,
